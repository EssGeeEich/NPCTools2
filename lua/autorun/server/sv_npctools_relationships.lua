...
local function StoreDisposition(src,tgt,disp)
	src.m_tbRelsDef = src.m_tbRelsDef or {}
	src.m_tbRelsDef[tgt] = src.m_tbRelsDef[tgt] or src:Disposition(tgt)
end
local function RestoreRelationship(src,tgt)
	if(not src.m_tbRelsDef or not src.m_tbRelsDef[tgt]) then return end
...
hook.Add("OnEntityCreated","npctool_relman_apply",function(ent)
	if(enabled and ent:IsValid() and ent:IsNPC()) then
		timer.Simple(0.1,function()
			if(ent:IsValid()) then
				...
				for _,entTgt in ipairs(ents.GetAll()) do
					if(entTgt:IsNPC() and entTgt ~= ent) then
						local classTgt = entTgt:GetClass()
						if(tRels[classTgt] and tRels[classTgt][class]) then
